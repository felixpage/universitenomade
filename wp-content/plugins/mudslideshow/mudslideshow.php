...
/*  Copyright 2007-2013  Sebaxtian  (email : baxtian.echeverry@gmail.com)
...
function muds_search($owner, $album, $url) {
	...
	switch($the_query->post_count) {
		...
		case 1:
			...
			break;
		default:
			$answer = get_bloginfo( 'url')."?s=$owner,$album";
			break;
	}
...
function muds_content($content) {
   ...
   $content = preg_replace($search, $replacement, $content);*/
	...
	$search = "/(?:<p>)*\s*\[mudslide:(picasa|flickr),(\d+),(htt[^,\]]+|([\w.\-\+@]+),([\w.\-\+@]+|\d+|[\w#]+))(,\d+,\d+|,(\d+c|\d+|s|t|m|l|full),(left|right|center)|)?\]\s*(?:<\/p>)*/i";
	if(preg_match_all($search, $content, $matches)) { //search for mudslide picasa
		if (is_array($matches)) { //We found
			foreach ($matches[1] as $key =>$v0) { //for each gallery, do
	...
	//MudSlideShow - Photo
	$search = "/(?:<p>)*\s*\[mudslide:(picasa|flickr),(\d+),(htt[^,]+|([\w.\-\+@]+),([\w.\-\+@]+|\d+|[\w#]+)),(\d+),(\d+|\d+c|s|t|m|l|full),(left|right|center)?\]\s*(?:<\/p>)*/i";
...
function muds_gallery($data, $update_post, $first=0, $last=0, $conf=0, $type, $rand=0) {
	global $wpdb, $post, $current_user, $wp_query, $muds;
			...
			if($muds_src = muds_post_thumbnail($postID)) {
				...
				switch($muds_src[3]) {
					...
					case 'flickr':
						...
						$path = $url['path'];
						$name = pathinfo($path);
						$name = str_replace("%", "", rawurldecode($name['basename']));
...
function muds_wpbook( $attachment, $post_id ) {
	...
	if($image) {
		...
		$attachment['picture'] = 'http://www.noimage.faketld';
	}
	return $attachment;
}
