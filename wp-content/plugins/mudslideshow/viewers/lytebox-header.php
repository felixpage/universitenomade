<?php 
	...
	//Declare javascript
	wp_register_script('lytebox', muds_plugin_url('/3rdparty/lytebox/lytebox.js'), array('jquery'), MUDS_HEADER_V);
	wp_enqueue_script('lytebox');
	
	//Define custom CSS URI
	$css = get_theme_root()."/".get_template()."/3rdparty/lytebox/lytebox.css";
	if(file_exists($css)) {
		$css_register = get_bloginfo('template_directory')."/lytebox.css";
	} else {
		$css_register = muds_plugin_url("/3rdparty/lytebox/lytebox.css");
	}
	//Declare style
