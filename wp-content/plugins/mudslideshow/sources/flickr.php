...
define('MUDS_FLICKR_API', 'b0db9e8f1370616609936df2a8d789f7');
...
function mudsFlickr_update_zero( $user ) {
	...
	$url = "http://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=". MUDS_FLICKR_API . "&user_id=$user&per_page=$per_page&extras=url_t,url_m,url_z,url_l,url_o,description";
	...
	if($album>0) { //If it is a number
		...
		if($data = muds_readfile($url)) { //Ask flickr
			...
			if($attr->stat=='ok') { //The answer is correct 
				...
				$url = "http://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=". MUDS_FLICKR_API . "&photoset_id=$album&extras=url_t,url_m,url_z,url_l,url_o,description";
...
function mudsFlickr_update_tag( $user, $tag ) {
	...
	$url = "http://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=". MUDS_FLICKR_API . "&user_id=$user&tags=".urlencode($tag)."&extras=url_t,url_m,url_z,url_l,url_o,description";
	$gallery_title=__('Album doesn\'t exist', 'mudslide' ); //Maybe we are wrong, so this is to create a dummy answer
	$out=false;
	if($data = muds_readfile($url)) { //Ask flickr
		$data = new SimpleXMLElement($data); //Parse the XML
...
function mudsFlickr_show_gallery( $user, $gallery, $first=0, $last=0, $conf=0, $rand=0 ) {
	...
	if(!muds_not_readable_file($filename)) { //If it's a readable gallery
		...
		$answer = muds_gallery($filename, $update_post, $first, $last, $conf, MUDS_TYPE_FLICKR, $rand);
...
function mudsFlickr_show_photo( $user, $gallery, $p_id, $size='t', $float=false, $conf=0 ) {
	...
	if(!muds_not_readable_file($filename)) { //If it's a readable file
		...
		$answer = muds_photo($filename, $p_id, $size, $float, $conf, MUDS_TYPE_FLICKR);
...
function mudsFlickr_widget_photo($filename, $p_id, $size='t') {
	...
	if(!muds_not_readable_file($filename)) { //If it's a readable file
		...
		//We have data, so we can try to show the photo.
		$answer = muds_photo($filename, $p_id, $size, 'center', 0, MUDS_TYPE_FLICKR, true);
	} else { 
...
function mudsFlickr_thumbnail($user, $gallery, $num) {
	...
	if($pass) {
		...
		if(isset($muds[$filename])) {
			$photo_gallery = $muds[$filename];
		} else {
			$data = get_transient($filename);
			$photo_gallery = new SimpleXMLElement($data);
			$muds[$filename] = $photo_gallery;
		}
